...
**ORM(Object Relational Mapping)** : 자바 객체와 데이터베이스를 연결하는 프로그래밍 기법
...
엔티티를 관리하는 객체로, 데이터베이스와 애플리케이션 사이에서 **CRUD 역할** 수행

### 엔티티 매니저 팩토리

엔티티 매니저를 생성하는 객체로, 여러 사용자의 요청에 따라 각각의 엔티티 매니저 생성
...
스프링 부트는 내부적으로 엔티티 매니저 팩토리를 하나만 생성하여 관리.
`@PersistenceContext` 또는 `@Autowired` 애너테이션으로 주입받아 사용.
...
JPA가 엔티티를 관리하는 **가상의 공간**으로, 엔티티 매니저는 엔티티를 이 공간에 저장.
...
엔티티는 영속성 컨텍스트와의 관계에 따라 4가지 상태를 가짐
...
`JpaRepository` 인터페이스를 통해 기본 CRUD 메서드를 자동으로 제공
...
복잡한 쿼리나 성능 최적화가 필요한 경우 JPQL을 직접 작성.
...
> `deleteAll()`은 모든 데이터를 삭제하므로 실제 서비스 코드에서는 사용 주의.  
> 주로 테스트 환경의 데이터 초기화 목적으로 사용.
...
JPA에서는 별도의 update 메서드 없이, **영속 상태 엔티티의 값을 변경하면 변경 감지로 자동 반영**
...
> `@DataJpaTest`는 기본적으로 트랜잭션이 포함되어 있어 테스트에서는 별도 선언 없이도 동작
